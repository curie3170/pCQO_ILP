...
def extract_params_from_csv(graph_dir):
    ...
    csv_dir = os.path.normpath(os.path.join("..", "fobc", "fast_tune", relative_suffix))
...
def get_dataset_name_from_path(graph_dir, lr, mom, gamma, gamma_prime):
    ...
    return f"er_graphs_N_{N}_{p}_{lr}_{mom}_{gamma}_{gamma_prime}", f"ER_{N}_0.{p}"

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python benchmark_ER_N_p_Gurobi_w0.15_repeated_fixed_cpp.py <clq graph_directory>, e.g python benchmark_ER_N_p_Gurobi_w0.15.py '/export3/curiekim/fobc/graphs/er_graphs/N_3000/30' ")
        sys.exit(1)
    ...
    #### EXECUTE CPP ####
    for clq_path in Path(graph_dir).glob("*.clq"):
        cmd = [
            pcqo_exec,
            str(clq_path),
            str(lr),
            str(mom),
            "500000",  # number_of_steps
            "500",     # steps_per_batch
            str(gamma),
            str(gamma_prime),
            "256",     # batch size
    ...
    base_solvers = [
        {"name": "Gurobi_warm 500", "class": GurobiMIS_warm, "params": {"time_limit": 30, "dataset": intermediate_results, "iteration": 500, "warm_sample_rate": 0.15}},
        {"name": "Gurobi_warm 30s", "class": GurobiMIS_warm, "params": {"time_limit": 30, "dataset": intermediate_results, "iteration": 0, "warm_sample_rate": 0.15}}, #pcqo30sec value
    ]

    solvers = base_solvers
    # Assemble dataset from .gpickle files in the specified directories
    graph_dir = graph_dir.replace("/fobc/", "/pCQO-mis-benchmark/")

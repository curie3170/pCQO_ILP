...
class pCQOMIS_MGD(Solver):
    ...
    def __init__(self, G: Graph, G_name, params):
        ...
        self.set_of_params = params.get("set_of_params", None)
    ...
    def solve(self):
        ...
        if self.value_initializer == "random":
            ...
            # ###Test degree##
            # with open("/export2/curiekim/pCQO-mis-benchmark/intermediate_results/er_700-800_30sec_hyperparam_bestbatch/ER_700_800_0.15_0_450.csv", "r") as file:
            #     numbers = [int(line.strip()) for line in file]
            # values = [degrees[k] for k in numbers if k in degrees]
        ...
        set_of_param = self.set_of_params[0]
        learning_rate, momentum, gamma, gamma_prime= set_of_param
        gamma = torch.tensor(gamma, device=device)
        gamma_prime = torch.tensor(gamma_prime, device=device)
        learning_rate = torch.tensor(learning_rate, device=device)
        momentum = torch.tensor(momentum, device=device)
        ...
        for iteration_t in range(number_of_iterations_T):
            ...
            Matrix_X_temp = Matrix_X
            # if (Matrix_X_pos > 0).sum().item() != 0:
            #     confidence = torch.quantile(Matrix_X_pos[Matrix_X_pos>0], self.confidence_th)
            #     confidence_mask = Matrix_X >= confidence
            # else:
            #     confidence_mask = torch.ones_like(Matrix_X, dtype=torch.float32, device='cuda')
                ...
                for batch_id, X_torch_binarized in enumerate(masks):
                    if X_torch_binarized.sum() != 0 and (X_torch_binarized.T @ adjacency_matrix_tensor @ X_torch_binarized) == 0:
                        ...
                        if torch.equal(X_torch_binarized, X_torch_binarized_update):
                            ...
                            #print(Matrix_X_temp[batch_id][MIS])
                            ...
                            if len(MIS) > best_MIS:
                                ...
                                best_MIS_list = MIS
                                #print(MIS)
                                best_MIS_masked = torch.nonzero(X_torch_binarized).squeeze()

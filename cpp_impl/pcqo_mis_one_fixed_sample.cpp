...
std::pair<std::vector<Parameters>, int> FindParameterRange(const std::string& file_path, int tuning_num_iter=1000)
{
    ...
    torch::manual_seed(113); // 고정된 시드로 실험을 실행
    auto [adj, adj_comp, n] = load_graph(file_path);
    InitializationSampler sampler(compute_mean_vector(adj, INITIALIZATION_VECTOR, batch_size, n), std);

    // 한 번만 X 샘플을 생성하고 이를 계속 사용
    torch::Tensor X = sampler.sample(torch::zeros({batch_size, n}, default_tensor_options_gpu));
    ...
    while (best_params_list.empty())
    {
        ...
        for (int gamma_prime : gamma_primes)
        {
            ...
            // Optimizer를 생성하여, X 샘플을 활용
            Optimizer opt(lr, mom, gamma, gamma_prime, n, batch_size);
            int local_max = 0;
            auto ones = torch::ones({n}, default_tensor_options_gpu);
            auto update = n * adj - adj_comp;

            for (int i = 0; i < num_iterations; ++i)
            {
                auto grad = opt.compute_gradient(adj, adj_comp, X);
                X = opt.velocity_update(X, grad).clamp(0, 1);

                if ((i + 1) % num_iterations_per_batch == 0)
                {
                    auto masks = X.gt(0.5).to(torch::kFloat16);

                    for (int j = 0; j < masks.size(0); ++j)
                    {
                        auto binarized = masks[j] - 0.1 * (-ones + masks[j].matmul(update));
                        binarized.clamp_(0, 1);

                        if (torch::equal(binarized, masks[j]))
                        {
                            int score = masks[j].sum().item<float>();
                            if (score > local_max)
                                local_max = score;
                        }
                    }

                    X = sampler.sample_previous(X);  // 이걸 한 번만 샘플링하도록 하기
                }
            }
        ...
        if (best_params_list.empty())
        {
            current_iteration = 0;
            num_iterations_per_batch *= 2;
            num_iterations *= 2;
            gammas.push_back(gammas.back() * 2);
        }
    }
